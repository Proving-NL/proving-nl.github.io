$().on('load', async e => {
  function api(path, options) {
    return aim.fetch('https://dms.aliconnect.nl/api/v1/abis'+path).headers({
  ...
  function start(){
    async function producten(search) {
      searchfunction = producten
      const [rows] = await api('/producten').query({
    ...
    let searchfunction = producten;

    $(document.body).text('').append(
      $('div').class('card'),
      $('nav').append(
        // $('button').text('contacten').on('click', e => contacten()),
        // $('a').text('< Login').on('click', e => signin()),
      ),
...
window.addEventListener( 'scroll', e => {
  if (window.scrollYdir != (window.scrollYdir = (window.scrollYdiff = (window.scrollYprev||0) - (window.scrollYprev = window.scrollY)) > 0)) {
    document.querySelector('div.search').style.top = (window.scrollYdir ? 0 : -50)+'px';
  };
});
