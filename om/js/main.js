...
Aim.assign({
	...
	api: Api = api = {
		...
		onload: function (row, i, rows) {
			...
			if (item.master) {
				if (item.master.children.indexOf(item) == -1) {
					item.master.children.push(item);
				}
				if (!(item.schema in item.master)) Object.defineProperty(item.master, item.schema, {
					...
					get: function (className) {
						...
						a.sort(App.sort.idx);
						return a;
					}.bind(item.master, item.schema),
					//set: this.setAttribute.bind(this, attributeName),
				});
				if (item.typical && !(item.typical in item.master)) Object.defineProperty(item.master, item.typical, {
					//get: this.getAttributeValue.bind(this, attributeName),
					get: function (className) {
						var a = [];
						this.children.forEach(function (child) { if (child.schema == className) a.push(child); });
	...
	load: function (par) {
		...
		var src = par.src ? (par.src[0] == '/' ? apiorigin : "") + par.src : (apiorigin + "/api1/" + (par.api || ''));
		...
		//console.log('ggggggg', src, par.get);
				...
				if (this.data.value && Array.isArray(this.data.value)) this.data.value.forEach(function (row) {
					...
					if (row.values) {
						...
						for (var attributeName in row.values) if (row.values[attributeName] != undefined) {
							...
							api.item[row.id].values[attributeName].value = value;
						}
...
libroot = '/lib';//apiorigin + "/aim/" + Aim.version + "/lib";
apiroot = '/api1/';//apiorigin + "/" + Aim.client.domain.name + "/" + Aim.version + "/api/";
